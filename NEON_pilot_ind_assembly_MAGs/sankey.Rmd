...
neon_jgi_pilot_ind_GTDB <- read_csv("neon-jgi_pilot_ind_assembly_bins.csv") %>% 
  ...
  unite(col = "classification", c(Domain, Phylum, Class, Order, Family, Genus), sep='; ') %>% 
...
neon_jgi_pilot_ind_GTDB_allTaxa <- bind_rows(neon_jgi_pilot_ind_GTDB_g, neon_jgi_pilot_ind_GTDB_f, neon_jgi_pilot_ind_GTDB_o, neon_jgi_pilot_ind_GTDB_c, neon_jgi_pilot_ind_GTDB_p, neon_jgi_pilot_ind_GTDB_d) %>% 
  mutate(classification = as.factor(classification)) %>% 
  count(classification) %>% 
# rename for Pavian format
  rename(`#SampleID` = `classification`) %>% 
