...
``` {r 'read data from Portal', message=FALSE, results='hide', eval = FALSE}
...
soil_fieldBgc <- select(soil_fieldBgc, sampleID, cnSampleID, bgcArchiveID,bgcSampleCondition=sampleCondition, bgcDataQF)
...
soil_chem <- soil_fieldL1$sls_soilChemistry %>% 
...
#soil_iso <- soil_fieldL1$bgc_CNiso_externalSummary %>%
...
#soilNtrans <- def.calc.ntrans(kclInt = soil_fieldL1$ntr_internalLab, 
#                       kclIntBlank = soil_fieldL1$ntr_internalLabBlanks, 
#                       kclExt = soil_fieldL1$ntr_externalLab, 
#                       soilMoist = soil_fieldL1$sls_soilMoisture)
...
# soilChem <- left_join(soil_chem, soilNtrans, by=c('sampleID'))

# join soil chem and field data tables
# soilFieldChem <- left_join(soilField, soilChem, by=c('sampleID', 'cnSampleID') )
soilFieldChem <- left_join(soilField, soil_chem, by=c('sampleID', 'cnSampleID') )
```
